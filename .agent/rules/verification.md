---
trigger: always_on
description: Enforce verification after code changes.
---
...
### Required Steps:
1. **Run the Verification Workflow**: Execute the `/verify_changes` slash command (or `./scripts/validate.sh`).
...
> [!IMPORTANT]
> A task is NOT complete until all checks (Formatting, Linting, Tests, and Build) pass successfully in the project's development environment.